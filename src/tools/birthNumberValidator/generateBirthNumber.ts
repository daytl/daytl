...
const parseCharToNumber = (str: string, charPosition: number): number =>
  Number(str.slice(charPosition));
...
export const isValidBirthNumber = (birthNumber: string): boolean => {
  ...
  if (Number.isNaN(firstNineNumericChars)) {
    return false;
  }
...
export const parseBirthNumber = (
  ...
  const [yearPart, monthPart, dayPart] = [
    birthNumber.slice(0, 2),
    birthNumber.slice(2, 4),
    birthNumber.slice(4, 6),
  ];
  let [year, month, day] = [parseInt(yearPart, 10), parseInt(monthPart, 10), parseInt(dayPart, 10)];
