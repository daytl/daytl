...
export const Base64EncodeDecode = () => {
  ...
  const onDrop = useCallback(acceptedFiles => {
    ...
    reader.onload = () => {
      setResult(stripHeader ? reader.result.replace(/^data:[^;]+;base64,/, "") : reader.result)
    }
  ...
  return <>
    <Grid container spacing={1}>
      ...
      <Grid item xs={12}>
        ...
        <FormControlLabel control={<Checkbox checked={stripHeader} onChange={
          (e) => setStripHeader(e.target.checked)
        } />}
                          labelPlacement="start"
                          label={<FormattedMessage namespace="tools" id="base64-encode-decode.stripHeader" />}
        />
      </Grid>
      <Grid item xs={12}>
        <TextField
